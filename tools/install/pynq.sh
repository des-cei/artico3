...
# Modify bootargs
BOOTARGS="bootargs = \"console=ttyPS0,115200 rootdelay=3 root=/dev/mmcblk0p2 rw earlyprintk uio_pdrv_genirq.of_id=generic-uio\";"
...
# Enable XDEVCFG to perform dynamic clock management (from version
...
echo "CONFIG_XILINX_DEVCFG=m" >> $WD/linux-xlnx/.config
...
dd if=/dev/zero of=linaropynq.img bs=1M count=4k status=progress

# Mount SD image file in loop device
sudo -H losetup -D
sudo -H losetup -P /dev/loop0 linaropynq.img
...
cp $WD/u-boot-digilent/spl/boot.bin $WD/rootfs/boot
cp $WD/u-boot-digilent/u-boot.img $WD/rootfs/boot
cp $WD/linux-xlnx/arch/arm/boot/uImage $WD/rootfs/boot
cp $WD/devicetree/pynq/dts/devicetree.dtb $WD/rootfs/boot
sync

# Download Linaro (Debian) and extract rootfs
wget http://releases.linaro.org/debian/images/developer-armhf/17.08/linaro-stretch-developer-20170706-43.tar.gz
sudo -H tar --strip-components=1 -xzvphf linaro-stretch-developer-20170706-43.tar.gz -C root
sync
...
# Load XDEVCFG kernel module
modprobe xilinx_devcfg

# Unbind PCAP from FPGA manager
echo f8007000.devcfg > /sys/bus/platform/drivers/zynq_fpga_manager/unbind
echo f8007000.devcfg > /sys/bus/platform/drivers/xdevcfg/bind

# Modify FPGA operating frequency
echo fclk0 > /sys/class/xdevcfg/xdevcfg/device/fclk_export
echo "Current operating frequency is \$(cat /sys/class/xdevcfg/xdevcfg/device/fclk/fclk0/set_rate)"
echo \$FREQ > /sys/class/xdevcfg/xdevcfg/device/fclk/fclk0/set_rate
echo "New operating frequency is \$(cat /sys/class/xdevcfg/xdevcfg/device/fclk/fclk0/set_rate)"

# Bind PCAP to FPGA manager
echo f8007000.devcfg > /sys/bus/platform/drivers/xdevcfg/unbind
echo f8007000.devcfg > /sys/bus/platform/drivers/zynq_fpga_manager/bind

# Remove XDEVCFG kernel module
modprobe -r xilinx_devcfg
EOF
